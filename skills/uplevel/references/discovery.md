...
on workflow files under `course/content/` — teaching material, not its pipeline. Exclude the obvious
...
This is the highest-consequence measurement error available: *what has already gone wrong here* is one
of the three questions the whole audit is aimed by, and a false clean there aims everything downstream
at the wrong thing.
...
- **Do the documented commands still work?** Paths rot: tool versions move, scripts are relocated,
  endpoints change. Run what the README claims before you copy it forward.
...
Run the preflight **first**: it decides how much of the audit is verifiable at all. When the gate
cannot run, everything downstream degrades to `unverified` — and a report that does not say so plainly
has quietly stopped being worth reading. `evidence.md` covers what to do once you know.
...
On a monorepo this lists every workspace — twenty or more is common — and most declare nothing. Only
the manifests carrying `packageManager`, `engines`, or a language version matter — filter to those:
...
`npm view yarn version`, `npm outdated -g`, and `brew upgrade` all report "up to date" on a machine
four majors behind what the repo pins. Only the project's declaration, and the tool as resolved in
the project's directory, count.
